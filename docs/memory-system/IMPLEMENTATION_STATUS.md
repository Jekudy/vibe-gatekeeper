...
| T1-02  | ingestion_runs table                                        | done           | Sprint 7 / PR #TBD. Alembic migration `004_add_ingestion_runs` (id pk, run_type non-null + check live/import/dry_run/cancelled, source_name nullable, started_at default now, finished_at nullable, status default 'running' + check, stats_json/config_json/error_json; indexes (run_type, started_at) and (status)). New `bot/db/models.py::IngestionRun` and `bot/db/repos/ingestion_run.py::IngestionRunRepo` with `create / update_status / get_active_live` methods. Validates run_type and status against allowed sets. `update_status` to terminal status sets `finished_at = now(UTC)`. Tests under `tests/db/test_ingestion_run_repo.py` (8 DB-backed + 1 metadata smoke). |
